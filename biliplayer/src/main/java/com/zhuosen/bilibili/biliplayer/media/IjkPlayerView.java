...
import com.zhuosen.bilibili.biliplayer.danmaku.BaseDanmakuConverter;
import com.zhuosen.bilibili.biliplayer.danmaku.BiliDanmukuParser;
import com.zhuosen.bilibili.biliplayer.danmaku.OnDanmakuListener;
import com.zhuosen.bilibili.biliplayer.utils.AnimHelper;
import com.zhuosen.bilibili.biliplayer.utils.MotionEventUtils;
import com.zhuosen.bilibili.biliplayer.utils.NavUtils;
import com.zhuosen.bilibili.biliplayer.utils.NetWorkUtils;
import com.zhuosen.bilibili.biliplayer.utils.SDCardUtils;
import com.zhuosen.bilibili.biliplayer.utils.SoftInputUtils;
import com.zhuosen.bilibili.biliplayer.utils.StringUtils;
import com.zhuosen.bilibili.biliplayer.utils.WindowUtils;
import com.zhuosen.bilibili.biliplayer.widgets.MarqueeTextView;
import com.zhuosen.bilibili.biliplayer.widgets.ShareDialog;
...
import static com.zhuosen.bilibili.biliplayer.utils.StringUtils.generateTime;
import static tv.danmaku.ijk.media.player.IMediaPlayer.OnInfoListener;

/**
 * Created by
 *
 *   tv_open_edit_danmaku  //弹幕输入框
 * 	iv_danmaku_control    //弹幕显示和隐藏按钮
 * 	2428 行
 */
public class IjkPlayerView extends FrameLayout implements View.OnClickListener {
    ...
    // 默认隐藏提示栏时间
    private static final int DEFAULT_TIPS_TIMEOUT = 8000;
    // 更新进度消息
    ...
    public MarqueeTextView mSmallTitle;
    ...
    private void _initView(Context context) {
        ...
        mSmallTitle = findViewById(R.id.small_title);
        // 视频宽高比设置
     ...
     *  设置小标题,需求
     */
    public IjkPlayerView setSmallTiele(String title) {
        mSmallTitle.setText(title);
        return this;
    }
...
//        mPlayerThumb.setVisibility(View.GONE);
    ...
    /**
     *   暴露一个方法,当当前进度达到某一个峰值的时候,抛出一个tag
     */
...
//            mHandler.postDelayed(mHideSkipTipRunnable, DEFAULT_HIDE_TIMEOUT * 2);
            mHandler.postDelayed(mHideSkipTipRunnable, DEFAULT_TIPS_TIMEOUT);
        }
    }
...
//            mDanmakuContext.setDanmakuSync(new VideoDanmakuSync(this));
    ...
    private void _toggleDanmakuView(boolean isShow) {
        if (mIsEnableDanmaku) {
            if (isShow) {
                mIvDanmakuControl.setVisibility(INVISIBLE);//  弹幕显示和隐藏按钮的显示
                mTvOpenEditDanmaku.setVisibility(INVISIBLE);// 弹幕输入框隐藏
    ...
    private boolean _isVideo(String str) {
