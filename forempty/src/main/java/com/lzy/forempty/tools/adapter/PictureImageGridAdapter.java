package com.lzy.forempty.tools.adapter;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (getItemViewType(position) == PictureConfig.TYPE_CAMERA) {
            ...
            if (mimeType == PictureMimeType.ofAudio()) {
                ...
                Drawable drawable = ContextCompat.getDrawable(context, R.mipmap.picture_audio);
                StringUtils.modifyTextViewDrawable(contentHolder.tv_duration, drawable, 0);
            } else {
                Drawable drawable = ContextCompat.getDrawable(context, R.mipmap.video_icon);
                StringUtils.modifyTextViewDrawable(contentHolder.tv_duration, drawable, 0);
            ...
            if (mimeType == PictureMimeType.ofAudio()) {
                ...
                options.centerCrop();
                options.placeholder(R.mipmap.ic_placeholder); //  @mipmap/ic_placeholder
                Glide.with(context)
                        .asBitmap()
                        .load(path)
                        .apply(options)
